...
(function ($, w, d) {
  ...
  const app = {
    ...
    updateURL(handle, replace = false) {
      ...
      else if (w.location.pathname !== url)
        history.pushState({ handle }, title, url);
    },
    ...
    async renderPanelsFor(handle) {
      ...
      if (prefMount && typeof prefHTML === "string")
        prefMount.innerHTML = prefHTML;
      if (lookMount && typeof lookHTML === "string")
        lookMount.innerHTML = lookHTML;
    },

    // If slick slide *is* the anchor, return it; else find it inside.
    getAnchorFromSlide($slide) {
      return $slide.is(S.slideAnchor)
        ? $slide
        : $slide.find(S.slideAnchor).first();
    },
...
document.addEventListener("click", function (e) {
  ...
  const chunk = Math.max(
    parseInt(btn.getAttribute("data-chunk") || "8", 10),
    1,
  );
